...
Better Task Manager 1.1.0-preview.14 remains a single Windows desktop executable.
...
The Apps view groups rows by executable path and sums private-byte and working-set values across all PIDs in that group. Search, typed sorting, and selection operate on the grouped in-memory snapshot and remain active when Live monitoring replaces it. The Processes view remains per-PID. Both views expose their snapshot time and Apps exposes its contributing process count so the scopes are directly comparable. A working set contains private and shared pages, so summing per-PID working sets can count a shared page more than once.
