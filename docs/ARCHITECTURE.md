...
Better Task Manager 1.1.0-preview.21 remains a single Windows desktop executable.
...
Non-destructive UI preferences are stored as atomic JSON under `%LOCALAPPDATA%\BetterTaskManager`. The app restores a screen-clamped window size, maximized state, and refresh interval. Live enabled state is excluded so launching the app never begins background sampling unexpectedly.
